...
| LLM控制 | 自然语言描述→剪辑指令 | ✅ |
| 时间轴导出 | EDL/CSV/JSON/XML | ✅ |
| WebUI | FastAPI + HTML 前端 | ✅ |
...
| `--enhance` | 视频增强 (stabilize/denoise/color-grade) | - |
| `--export-timeline` | 导出时间轴 (edl/csv/json/xml) | - |
| `--webui` | 启动 WebUI | - |
...
├── packages/
│   ├── core_types/          # 统一数据类型
│   ├── video_crawler/       # 素材爬取（B站/YouTube/台词搜索）
│   ├── video_enhancement/   # 视频增强（防抖/降噪/调色）
│   ├── subtitle_engine/     # 字幕引擎（Whisper转录+压制）
│   ├── ai_director/         # LLM自然语言控制
│   ├── timeline_export/     # 时间轴导出（EDL/XML/CSV/JSON）
│   ├── webui/               # WebUI（FastAPI + HTML）
│   ├── beat_engine/         # 节拍引擎
